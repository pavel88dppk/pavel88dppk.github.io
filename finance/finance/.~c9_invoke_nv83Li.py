...
def index():
    ...
    for each_symbol in symbols:
        ...
        totalcash += total
        db.execute("UPDATE paymentsss SET price=:price, total=:total WHERE user_id=:id AND symbol=:symbol", price=stock["price"], total=total, id=session["user_id"], symbol=symbol)
...
def buy():
    ...
    if request.method == "POST":
        ...
        if (price < money[0]["cash"]):
            ...
            table = db.execute("SELECT symbol FROM paymentsss WHERE user_id = :id AND symbol = :symbol", id = session["user_id"], symbol = request.form.get("symbol"))
            
            if table:
                current = db.execute("SELECT shares, total FROM paymentsss WHERE symbol = :symbol AND user_id = :id", symbol = request.form.get("symbol"), id = session["user_id"])
                newshares = (current[0]["shares"] + int(request.form.get("shares")))
                newtotal = (current[0]["total"] + price)
                db.execute("UPDATE paymentsss SET shares = :newshares, total = :newtotal WHERE symbol = :symbol AND user_id = :id", newshares = newshares, newtotal = newtotal, symbol = request.form.get("symbol"), id = session["user_id"])
            else:
...
def register():
    ...
    if request.method == "POST":
        ...
        new_user = db.execute("INSERT INTO users (username, hash) VALUES(:username, :hash)", username=username, hash=password_hash)
...
def sell():
    ...
    if request.method == "POST":
        ...
        actualamount = db.execute("SELECT shares, SUM(shares) as total_shares FROM paymentsss WHERE user_id = :id AND symbol = :symbol GROUP BY symbol HAVING total_shares > 0", id = session["user_id"], symbol = symbolx)
        ...
        db.execute("UPDATE paymentsss SET shares = :aftershare WHERE user_id = :id AND symbol = :symbol", aftershare = after_share, id = session["user_id"], symbol = symbolx)

        flash("Sold!")

        return redirect("/")
    else:
        stocks = db.execute("SELECT symbol, SUM(shares) as total_shares FROM paymentsss WHERE user_id = :user_id GROUP BY symbol HAVING total_shares > 0", user_id=session["user_id"])
